...
pool = Pool(6)
import time
from db_upload import attach_listfields_to_records, update_one
import stanfordnlp
import re
import nltk
    ...
    for record in data:
        try:
            ...
            if new_pros_embeddings:
                update_one("pos_filtered_pros_embedding", new_pros_embeddings, record, coll)
                update_one("pos_filtered_pros_tokens", new_pros_tokens, record, coll)
        except KeyError:
            pass
        try:
            ...
            if new_cons_embeddings:
                update_one("pos_filtered_cons_embedding", new_cons_embeddings, record, coll)
                update_one("pos_filtered_cons_tokens", new_cons_tokens, record, coll)
        except KeyError:
            pass
        try:
            ...
            if new_title_embeddings:
                update_one("pos_filtered_title_embedding", new_title_embeddings, record, coll)
                update_one("pos_filtered_title_tokens", new_title_tokens, record, coll)
        except KeyError:
            pass
        k += 1
        if k%100 == 0:
...
def pool_pos_embeddings():
    ...
    while True:
        ...
        attach_listfields_to_records("mean_pooling_pos_filtered_pros", pro_means, records, coll)
        attach_listfields_to_records("max_pooling_pos_filtered_single_pros", pro_maxes_s, records, coll)
        attach_listfields_to_records("max_pooling_pos_filtered_total_pros", pro_maxes_t, records, coll)
        attach_listfields_to_records("mean_pooling_pos_filtered_cons", con_means, records, coll)
        attach_listfields_to_records("max_pooling_pos_filtered_single_cons", con_maxes_s, records, coll)
        attach_listfields_to_records("max_pooling_pos_filtered_total_cons", con_maxes_t, records, coll)
        attach_listfields_to_records("mean_pooling_pos_filtered_title", title_means, records, coll)
        attach_listfields_to_records("max_pooling_pos_filtered_single_title", title_maxes_s, records, coll)
        attach_listfields_to_records("max_pooling_pos_filtered_total_title", title_maxes_t, records, coll)
